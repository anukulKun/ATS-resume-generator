...
> Note: `npm run scan` runs `scripts/scan.mjs`. It uses public Greenhouse, Ashby, and Lever APIs without AI tokens. If a company does not use one of those supported APIs, the AI runner should check that company manually with browser/web search tools.
...
- **Greenhouse**: `"AI Engineer at Example AI Co"` -> title: `AI Engineer`, company: `Example AI Co`
...
→ Ejecuta /ATS Resume Generator pipeline para evaluar las nuevas ofertas.
```
