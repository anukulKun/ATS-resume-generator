#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for ATS Resume Generator pipeline integrity
 *
 * Checks:
 ...
 *
 * Run: node ATS-resume-generator/verify-pipeline.mjs
 */
...
for (const e of entries) {
  ...
  if (!match) continue;
  const reportPath = join(PROJECT_ROOT, match[1]);
